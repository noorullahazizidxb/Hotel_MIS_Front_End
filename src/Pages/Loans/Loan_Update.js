...
export default function Loan_Update(props) {
      ...
      <div className={classes.container}>
        <Zoom in={checked}>
          <Paper elevation={4} className={classes.paper}>
            <Loan_Form recordForEdit={rowData} customer_name={data.Customer_Name}/>
          </Paper>
        </Zoom>
...
const Loan_Form=withRouter((props)=>{
    // const { addOrEdit, recordForEdit } = props
    const { customer_name,recordForEdit,history} = props
    ...
    const Update_Loan=async (address,values)=>{
      
      await Api.put(address,values)
...
const get_data=async (address)=>{
  ...
  .then(function () {
    ...
    useEffect(async () => {
        ...
        values.Bill_Number=recordForEdit.Bill_Number;
        
        childRef.current.getAlert(customer_name);
        setBill_Number(recordForEdit.Bill_Number);
        setTotal_Bill_Money(recordForEdit.Total_Price);
...
const handleScanCam=async (result_Cam)=>{
  if(result_Cam){
    if(values.Customer_ID==''){
      values.Customer_ID=recordForEdit.Customer_ID;
    }
    console.log(values);
    if (validate()) {
      // addOrEdit(values, resetForm);
      ...
      Update_Loan('Loans/'+values.id,values);;
