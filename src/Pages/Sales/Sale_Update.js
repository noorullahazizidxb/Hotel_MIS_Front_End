...
export default function Sale_Update(props) {
      ...
      <div className={classes.container}>
        <Zoom in={checked}>
          <Paper elevation={4} className={classes.paper}>
            <Sale_Form recordForEdit={rowData} customer_name={data.Customer_Name}/>
          </Paper>
        </Zoom>
...
const Sale_Form=withRouter((props)=>{
    ...
    const Update_Sale=async (address,values)=>{
...
const handleScanCam=async (result_Cam)=>{
  if(result_Cam){
    ...
    if (validate()) {
      ...
      Update_Sale('Sales/'+values.id,values);
